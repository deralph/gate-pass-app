...
export const processScan = async (data:string, qrCodeData: any, studentStaffId: string) => {
  console.log("qrCodeData = ",qrCodeData, " studentStaffId = ",studentStaffId )
  try {
    const response = await api.post('/scans/process', {data,
      qrCodeData,
      studentStaffId
    });
...
export const updateScanResult = async (scanId: string, result: 'approved' | 'denied', isIn:boolean,reason?: string,) => {
  try {
